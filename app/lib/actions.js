...
export const deleteReport = async (formData) => {
  ...
  console.log(
    `Attempting to delete report with ID: ${id} and reportedItemId: ${reportedItemId}`
  );
...
export const addPin = async (formData) => {
  const {
    createdBy,
    businessName,
    description,
  ...
  permanentRedirect("/dashboard/pins");
};

export const updatePin = async (formData) => {
  const {
    id,
    businessName,
    description,
  ...
  try {
    ...
    const updatedPin = await Pin.findByIdAndUpdate(id, updateFields, {
      new: true,
    });

    if (!updatedPin) {
      throw new Error("Pin not found or update failed.");
    }
  } catch (err) {
    console.log(err);
...
export const deletePin = async (formData) => {
  const { id } = Object.fromEntries(formData);
